...
{      	
	...
	for (i = 0; i < 8; i++)
	{
		OLED_WriteData(~OLED_F8x16[Char - ' '][i]);			//显示上半部分内容，数据取反实现反色
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);	//设置光标位置在下半部分
	for (i = 0; i < 8; i++)
	{
		OLED_WriteData(~OLED_F8x16[Char - ' '][i + 8]);		//显示下半部分内容，数据取反实现反色
	}
}
...
void OLED_ShowSignedDecimal(uint8_t Line, uint8_t Column, float Number, uint8_t IntegerLength, uint8_t DecimalLength)
{
	...
	// 提取整数部分
	integerPart = (uint32_t)absNumber;
	
	// 提取小数部分
	decimalPart = (uint32_t)((absNumber - integerPart) * OLED_Pow(10, DecimalLength) + 0.5f); // 四舍五入
	
	// 显示整数部分
	for (i = 0; i < IntegerLength; i++)
	{
		OLED_ShowChar(Line, Column + 1 + i, integerPart / OLED_Pow(10, IntegerLength - i - 1) % 10 + '0');
	}
	
	// 显示小数点
	OLED_ShowChar(Line, Column + 1 + IntegerLength, '.');
	
	// 显示小数部分
	for (i = 0; i < DecimalLength; i++)
	{
		OLED_ShowChar(Line, Column + 1 + IntegerLength + 1 + i, decimalPart / OLED_Pow(10, DecimalLength - i - 1) % 10 + '0');
	}
}
